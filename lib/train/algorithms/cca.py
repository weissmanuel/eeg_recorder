...
def _msetcca_cal_template_U(X_single_stimulus: ndarray,
    ...
    for filterbank_idx in range(filterbank_num):
        ...
        for trial_idx in range(trial_num):
            template_temp = eig_vec[(trial_idx * channel_num):((trial_idx + 1) * channel_num),
                            :n_component].T @ X_single_stimulus_single_filterbank[trial_idx, :, :]
...
class MsetCCA(BaseModel):
    ...
    def fit(self,
        ...
        if self.n_jobs is not None:
            U_all_stimuli, template_all_stimuli = zip(*Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_msetcca_cal_template_U, I=np.eye(X[0].shape[-1])))(a) for a in separated_trainSig))
        else:
            U_all_stimuli = []
    ...
    def predict(self,
        ...
        if self.n_jobs is not None:
            r = Parallel(n_jobs=self.n_jobs)(delayed(
                partial(_r_cca_qr, n_component=self.n_component, Y_Q=template_sig_Q, Y_R=template_sig_R,
                        Y_P=template_sig_P, force_output_UV=False))(a) for a in X)
        else:
            r = []
            for a in X:
                r.append(
                    _r_cca_qr(a, n_component=self.n_component, Y_Q=template_sig_Q, Y_R=template_sig_R,
                              Y_P=template_sig_P, force_output_UV=False)
                )
        # self.model['U'] = U
...
class MsetCCAwithR(BaseModel):
    ...
    def fit(self,
        ...
        if self.n_jobs is not None:
            U_all_stimuli, template_all_stimuli = zip(*Parallel(n_jobs=self.n_jobs)(
                delayed(_msetcca_cal_template_U)(X_single_stimulus=a, I=Q @ Q.T) for a, Q in
                zip(separated_trainSig, ref_sig_Q)))
        else:
            U_all_stimuli = []
            template_all_stimuli = []
...
class OACCA(BaseModel):
    ...
    def predict(self,
        ...
        for x_single_trial in X:
            ...
            cca_r, cca_sfx, cca_sfy = _r_cca_canoncorr(x_single_trial, Y, n_component,
                                                       True)  # cca_sfx: (filterbank_num * stimulus_num * channel_num * n_component)
            ...
            if cca_res == oacca_res:
                ...
                if self.n_jobs is not None:
                    u0_list, new_covar_mat_list = zip(*Parallel(n_jobs=self.n_jobs)(
                        delayed(_oacca_cal_u0)(sf1x=sf1x, old_covar_mat=old_covar_mat) for sf1x, old_covar_mat in
                        zip(sf1x_list, old_covar_mat_list)))
                else:
                    u0_list = []
                    new_covar_mat_list = []
                ...
                self.model['covar_mat'] = np.concatenate(
                    [np.expand_dims(covar_mat, axis=2) for covar_mat in new_covar_mat_list], axis=2)
            ...
            if self.n_jobs is not None:
                u1_list, v1_list, new_Cxx_list, new_Cxy_list = zip(*Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(_oacca_cal_u1_v1, sinTemplate=Y[prototype_res][:, :signal_len]))(
                        filteredData=filteredData, old_Cxx=old_Cxx, old_Cxy=old_Cxy) for filteredData, old_Cxx, old_Cxy
                    in zip(filteredData_list, old_Cxx_list, old_Cxy_list)))
            else:
                u1_list = []
                v1_list = []
                new_Cxx_list = []
                ...
                for filteredData, old_Cxx, old_Cxy in zip(filteredData_list, old_Cxx_list, old_Cxy_list):
                    u1_temp, v1_temp, new_Cxx_temp, new_Cxy_temp = _oacca_cal_u1_v1(filteredData=filteredData,
                                                                                    old_Cxx=old_Cxx, old_Cxy=old_Cxy,
                                                                                    sinTemplate=Y[prototype_res][:,
                                                                                                :signal_len])
...
class SCCA_canoncorr(BaseModel):
    ...
    def __copy__(self):
        copy_model = SCCA_canoncorr(sfreq=self.sfreq,
    ...
    def get_ref_sig(self):
        return generate_harmonics(frequencies=self.target_frequencies,
                                  duration=self.signal_duration_seconds,
                                  sfreq=self.sfreq,
    ...
    def predict(self, X: List[ndarray]) -> List[int]:
        ...
        if update_UV or self.model['U'] is None or self.model['V'] is None:
            if force_output_UV or not update_UV:
                if self.n_jobs is not None:
                    r, U, V = zip(*Parallel(n_jobs=self.n_jobs)(
                        delayed(partial(_r_cca_canoncorr, n_component=n_component, Y=Y, force_output_UV=True))(a) for a
                        in X))
                else:
                    r = []
                    U = []
                ...
                if self.n_jobs is not None:
                    r = Parallel(n_jobs=self.n_jobs)(
                        delayed(partial(_r_cca_canoncorr, n_component=n_component, Y=Y, force_output_UV=False))(a) for a
                        in X)
                else:
                    r = []
                    for a in X:
            ...
            if self.n_jobs is not None:
                r = Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(_r_cca_canoncorr_withUV, Y=Y))(X=a, U=u, V=v) for a, u, v in zip(X, U, V))
            else:
                r = []
...
class SCCA_qr(BaseModel):
    ...
    def __copy__(self):
        copy_model = SCCA_qr(sfreq=self.sfreq,
                             target_frequencies=self.target_frequencies,
                             signal_duration_seconds=self.signal_duration_seconds,
                             n_harmonics=self.n_harmonics,
                             n_component=self.n_component,
    ...
    def predict(self,
        ...
        if update_UV or self.model['U'] is None or self.model['V'] is None:
            if force_output_UV or not update_UV:
                if self.n_jobs is not None:
                    r, U, V = zip(*Parallel(n_jobs=self.n_jobs)(delayed(
                        partial(_r_cca_qr, n_component=n_component, Y_Q=Y_Q, Y_R=Y_R, Y_P=Y_P, force_output_UV=True))(a)
                                                                for a in X))
                else:
                    r = []
                    U = []
                    V = []
                    for a in X:
                        r_temp, U_temp, V_temp = _r_cca_qr(a, n_component=n_component, Y_Q=Y_Q, Y_R=Y_R, Y_P=Y_P,
                ...
                if self.n_jobs is not None:
                    r = Parallel(n_jobs=self.n_jobs)(delayed(
                        partial(_r_cca_qr, n_component=n_component, Y_Q=Y_Q, Y_R=Y_R, Y_P=Y_P, force_output_UV=False))(
                        a) for a in X)
                else:
                    r = []
                    for a in X:
            ...
            if self.n_jobs is not None:
                r = Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(_r_cca_qr_withUV, Y_Q=Y_Q, Y_R=Y_R, Y_P=Y_P))(X=a, U=u, V=v) for a, u, v in
                    zip(X, U, V))
            else:
                r = []
                for a, u, v in zip(X, U, V):
...
class ITCCA(BaseModel):
    ...
    def __copy__(self):
        copy_model = ITCCA(sfreq=self.sfreq,
                           target_frequencies=self.target_frequencies,
                           signal_duration_seconds=self.signal_duration_seconds,
                           n_harmonics=self.n_harmonics,
                           n_component=self.n_component,
...
class ECCA(BaseModel):
    ...
    def __copy__(self):
        copy_model = ECCA(sfreq=self.sfreq,
                          target_frequencies=self.target_frequencies,
                          signal_duration_seconds=self.signal_duration_seconds,
                          n_harmonics=self.n_harmonics,
                          n_component=self.n_component,
    ...
    def fit(self,
        ...
        for filterbank_idx in range(filterbank_num):
            if self.n_jobs is not None:
                U, V, _ = zip(*Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(canoncorr, force_output_UV=True))(X=template_sig_single[filterbank_idx, :, :].T,
                                                                      Y=ref_sig_single.T)
                ...
                V = []
                for template_sig_single, ref_sig_single in zip(template_sig, ref_sig):
                    U_temp, V_temp, _ = canoncorr(X=template_sig_single[filterbank_idx, :, :].T, Y=ref_sig_single.T,
                                                  force_output_UV=True)
                    U.append(U_temp)
                    V.append(V_temp)
    ...
    def predict(self,
        ...
        if update_UV or self.model['U1'] is None or self.model['V1'] is None:
            if self.n_jobs is not None:
                r1, U1, V1 = zip(*Parallel(n_jobs=self.n_jobs)(delayed(
                    partial(_r_cca_qr, n_component=n_component, Y_Q=ref_sig_Q, Y_R=ref_sig_R, Y_P=ref_sig_P,
                            force_output_UV=True))(a) for a in X))
            else:
                r1 = []
                U1 = []
                V1 = []
                for a in X:
                    r1_temp, U1_temp, V1_temp = _r_cca_qr(a, n_component=n_component, Y_Q=ref_sig_Q, Y_R=ref_sig_R,
                                                          Y_P=ref_sig_P, force_output_UV=True)
                    r1.append(r1_temp)
                    U1.append(U1_temp)
                    V1.append(V1_temp)
            self.model['U1'] = U1
            ...
            if self.n_jobs is not None:
                r1 = Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(_r_cca_qr_withUV, Y_Q=ref_sig_Q, Y_R=ref_sig_R, Y_P=ref_sig_P))(X=a, U=u, V=v) for
                    a, u, v in zip(X, U1, V1))
            else:
                r1 = []
        ...
        if update_UV or self.model['U2'] is None:
            if self.n_jobs is not None:
                _, U2, _ = zip(*Parallel(n_jobs=self.n_jobs)(delayed(
                    partial(_r_cca_qr, n_component=n_component, Y_Q=template_sig_Q, Y_R=template_sig_R,
                            Y_P=template_sig_P, force_output_UV=True))(a) for a in X))
            else:
                U2 = []
                for a in X:
                    _, U2_temp, _ = _r_cca_qr(a, n_component=n_component, Y_Q=template_sig_Q, Y_R=template_sig_R,
                                              Y_P=template_sig_P, force_output_UV=True)
                    U2.append(U2_temp)
            self.model['U2'] = U2

        if self.n_jobs is not None:
            r2 = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_r_cca_qr_withUV, Y_Q=template_sig_Q, Y_R=template_sig_R, Y_P=template_sig_P))(X=a, U=u,
                                                                                                               V=v) for
                a, u, v in zip(X, U2, U2))

            # r3
            r3 = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_r_cca_qr_withUV, Y_Q=template_sig_Q, Y_R=template_sig_R, Y_P=template_sig_P))(X=a, U=u,
                                                                                                               V=v) for
                a, u, v in zip(X, U1, U1))

            # r4
            r4 = Parallel(n_jobs=self.n_jobs)(delayed(
                partial(_r_cca_qr_withUV, Y_Q=template_sig_Q, Y_R=template_sig_R, Y_P=template_sig_P, U=U3, V=U3))(X=a)
                                              for a in X)
        else:
            r2 = []
        ...
        Y_pred = [int(np.argmax(weights_filterbank @ (np.sign(r1_single) * np.square(r1_single) +
                                                      ...
                                                      np.sign(r4_single) * np.square(r4_single)))) for
                  r1_single, r2_single, r3_single, r4_single in zip(r1, r2, r3, r4)]
        r = [(np.sign(r1_single) * np.square(r1_single) +
              np.sign(r2_single) * np.square(r2_single) +
              np.sign(r3_single) * np.square(r3_single) +
              np.sign(r4_single) * np.square(r4_single)) for r1_single, r2_single, r3_single, r4_single in
             zip(r1, r2, r3, r4)]
...
class MSCCA(BaseModel):
    ...
    def fit(self,
        ...
        for filterbank_idx in range(filterbank_num):
            if self.n_jobs is not None:
                U_tmp, V_tmp, _ = zip(*Parallel(n_jobs=self.n_jobs)(
                    delayed(partial(canoncorr, force_output_UV=True))(X=template_sig_single[filterbank_idx, :, :].T,
                                                                      Y=ref_sig_single.T)
                ...
                V_tmp = []
                for template_sig_single, ref_sig_single in zip(template_sig_mscca, ref_sig_mscca):
                    U_temp_temp, V_temp_temp, _ = canoncorr(X=template_sig_single[filterbank_idx, :, :].T,
                                                            Y=ref_sig_single.T, force_output_UV=True)
    ...
    def predict(self,
        ...
        if self.n_jobs is not None:
            r1 = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_r_cca_canoncorr_withUV, Y=ref_sig, U=U, V=V))(X=a) for a in X)
            r2 = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_r_cca_canoncorr_withUV, Y=template_sig, U=U, V=U))(X=a) for a in X)
        else:
            r1 = []
            for a in X:
                r1.append(
        ...
        Y_pred = [int(np.argmax(weights_filterbank @ (np.sign(r1_single) * np.square(r1_single) +
                                                      np.sign(r2_single) * np.square(r2_single)))) for
                  r1_single, r2_single in zip(r1, r2)]
...
class CCAClassifier(TransformerMixin, ClassifierMixin):
    ...
    def find_corr(self, X) -> ndarray:
        cca = CCA(self.n_components)
        corr = np.zeros(self.n_components)
        result = np.zeros(self.target_frequencies.shape[0])
        for freq_idx, freq in enumerate(self.target_frequencies):
            Xw_x, Yw_y = cca.fit_transform(X, self.ref_signal[freq_idx])
            for i in range(self.n_components):
                corr[i] = np.corrcoef(Xw_x[:, i], Yw_y[:, i])[0, 1]
            result[freq_idx] = np.max(corr)
        return result

    def fit(self, X, y):

        ref_signal = generate_harmonics(frequencies=self.target_frequencies.tolist(),
                                        duration=self.signal_duration_seconds,
                                        sfreq=self.sfreq,
                                        num_harmonics=self.n_harmonics)
        self.ref_signal = np.array(ref_signal)

    def predict(self, X):
        corr = self.find_corr(X)
